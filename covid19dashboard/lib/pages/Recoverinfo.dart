...
class Recoverinfo extends StatefulWidget {

  @override
  _RecoverinfoState createState() => _RecoverinfoState();
}

class _RecoverinfoState extends State<Recoverinfo> {
      ...
      backgroundColor: Colors.white,
        body:Container(
                    ...
                    child: Column(
                      children: [
                        Padding(
                          ...
                          child: Text("local Recovered", style: TextStyle(color: const Color(0xffFFFFFF),fontWeight: FontWeight.bold,fontSize: 20),),
                        ),
                        FutureBuilder <Welcome>(
                            future: _covidData,
                            builder: (context, snapshot) {
                              if(snapshot.hasData){
                                dataValue = snapshot.data!.data.localRecovered;
                                return Padding(
          ...
          Expanded(
            child: Container(
              color: Colors.red,
              child: Text("Chart for Local Rec and global Rec"),
            ),
          )
            ],
          ),
